classdef func
    methods(Static)
        ...
        function y = linear_stage(Day, Cost,Cost_imp,Demand,Hydro_cap, Thermal_cap,Hydro_avg,Ramp_Rate,Sol_E,n,t,Beta,flag,res,in)
...
% to be checked
            ...
            w =1; h=1;
            for i= 1:t
                ub(1,w+n*t) = res(1,w+n*t);                  lb(1,w+n*t) = res(1,w+n*t)-0.1;
                ub(1,w+t*(n+1)) = 10;           lb(1,w+t*(n+1)) = 0;
                ub(1,w+t*(n)+t*(n+2)) = res(1,w+t*(n)+t*(n+2));   lb(1,w+t*(n+1)+t*(n+2)) = res(1,w+t*(n)+t*(n+2))-0.1;
                ub(1,w+t*(n+1)+t*(n+2)) = 10;
                for j = 1:n
                    ub(1,h) = res(1,h);           lb(1,h) = res(1,h)-0.1;
                    ub(1,h+t*(n+2)) = res(1,h+t*(n+2));   lb(1,h+t*(n+2)) = res(1,h+t*(n+2))-0.1;
                    h = h+t;
                end
                w=w+1;h=w;
            end
            ...
            y = result;
            elseif (flag ==2)
                model.A = in.A + sparse(Cnstrns);
                model.obj = in.obj + Obj;
                model.rhs = in.rhs + RHS;
                model.sense = sense1;
                model.ub = in.ub + ub;
                model.modelsense = 'min';
                model.lb = in.lb + lb;
        ...
        function y = dynamic_stage(Cost,Dem,Sol_E,Beta,Vol_1,Rain,MDDL,FRL,Hp,t,p,Avg,V1_mu,flag,res,in)
            ...
            params.outputflag = 0;
            %disp(model);
            %disp(size(model.sense));
            y = gurobi(model,params); 
            %{
            result.objval = res*Obj'; 
            result.x = 1;
            y = result;
            %} 
           % disp(size(res));
